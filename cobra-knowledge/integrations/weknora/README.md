...
- **本体图**: reads the ontology bound to the current knowledge base and projects classes, data properties, class hierarchy and object relations into the same graph-view contract.
...
Run or build WeKnora from `build/weknora-v0.3`. The `upstream/weknora` checkout remains untouched, so a future upstream update is simply:

```bash
cd upstream/weknora
git pull
cd ../..
./cobra-knowledge/integrations/weknora/apply-overlay.sh upstream/weknora build/weknora-v0.3
```
