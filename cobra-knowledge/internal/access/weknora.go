...
// User-facing WeKnora bearer identities are intentionally not accepted in v0.8.
...
func (a *WeKnoraAccessChecker) CheckKnowledgeBaseAccess(ctx context.Context, kbID string, headers http.Header) error {
	if a.BaseURL == "" { return fmt.Errorf("weknora base url is not configured") }
	...
	for _, name := range []string{"X-API-Key", "X-Tenant-ID", "X-External-User-ID", "Accept-Language"} {
		if value := headers.Get(name); value != "" { req.Header.Set(name, value) }
	}
	resp, err := a.Client.Do(req)
	if err != nil { return err }
	defer resp.Body.Close()
	_, _ = io.Copy(io.Discard, io.LimitReader(resp.Body, 1<<20))
	if resp.StatusCode < 200 || resp.StatusCode >= 300 { return fmt.Errorf("weknora access check returned %d", resp.StatusCode) }
	return nil
}
