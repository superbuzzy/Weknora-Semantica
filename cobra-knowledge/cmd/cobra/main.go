...
func discoverCmd(args []string) error {
	...
	if err := store.ReadJSON(*in, &g); err != nil { return err }
	o, r, err := ontsvc.NewDiscoveryService(nil).Discover(context.Background(), g)
	if err != nil { return err }
	if err := store.WriteJSON(*report, r); err != nil { return err }
...
func compileCmd(args []string) error {
	...
	if err := store.ReadJSON(*in, &o); err != nil { return err }
	v := ontsvc.ValidateOntology(o)
	for _, x := range v {
		if x.Severity == "error" { return fmt.Errorf("ontology validation failed: %s", x.Message) }
	}
...
func planCmd(args []string) error {
	...
	if *overlayPath != "" {
		if err := store.ReadJSON(*overlayPath, &overlay); err != nil { return err }
	}
...
func arbitrateCmd(args []string) error {
	...
	if err := store.ReadJSON(*in, &as); err != nil { return err }
	...
	if *policyPath != "" {
		if err := store.ReadJSON(*policyPath, &policy); err != nil { return err }
	}
	qt := time.Now().UTC()
	if *at != "" {
...
func bootstrapCmd(args []string) error {
	...
	files := []struct{name string; v interface{}}{{"normalized-graph.json", g}, {"resolution.json", rr}, {"pattern-report.json", report}, {"candidate-ontology.json", o}, {"weknora-extract-config.json", ontsvc.CompileWeKnora(o)}, {"assertions.json", assertions}}
	for _, f := range files {
		if err := store.WriteJSON(*dir+"/"+f.name, f.v); err != nil { return err }
	}
	summary := map[string]interface{}{"version": "0.9.0", "entities": len(g.Entities), "relations": len(g.Relations), "classes": len(o.Classes), "properties": len(o.Properties), "ontology_relations": len(o.Relations), "review_items": len(o.ReviewQueue)}
...
func registryActivateCmd(args []string) error {
	...
	actor := fs.String("actor", "cli", "actor")
	if err := fs.Parse(args); err != nil { return err }
	if err := ontsvc.NewFSRegistry(*root).Activate(context.Background(), *ontologyID, *version, *actor); err != nil { return err }
...
func registryBindCmd(args []string) error {
	...
	if err := r.BindKnowledgeBase(context.Background(), binding); err != nil { return err }
...
func registryResolveCmd(args []string) error {
	...
	if err != nil { return err }
	if *out != "" {
		if err := store.WriteJSON(*out, resolution.Ontology); err != nil { return err }
	}
